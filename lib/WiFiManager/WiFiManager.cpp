...
void WiFiManager::handleRoot() {
  ...
  page.replace("{v}", "Configuration | Smart Meter");
  page += FPSTR(HTTP_SCRIPT);
  page += FPSTR(HTTP_STYLE);
  page += _customHeadElement;
  page += FPSTR(HTTP_HEADER_END);
  page += String(F("<h1>"));
  page += "Smart Meter configuration";
  page += String(F("</h1>"));
...
void WiFiManager::handleWifi(boolean scan) {
  ...
  String page = FPSTR(HTTP_HEADER);
  page.replace("{v}", "WiFi | Smart Meter");
  page += FPSTR(HTTP_SCRIPT);
  page += FPSTR(HTTP_STYLE);
  page += _customHeadElement;
  page += FPSTR(HTTP_HEADER_END);
...
boolean WiFiManager::captivePortal() {
  if (!isIp(server->hostHeader()) && server->hostHeader() != this->hostname) {
    DEBUG_WM(F("Request redirected to captive portal"));
    server->sendHeader("Location", String("http://") + this->hostname);
